...
高温恢复要求 `≤27.5°C` 持续 15 分钟；低温恢复要求 `≥23.5°C` 持续 15 分钟。
快速温变必须连续 30 分钟窗口变化 `<1.0°C` 满 30 分钟才恢复，并按该事件曾达到的
最高等级通知一次；循环异常须温差 `≤0.8°C` 持续 10 分钟才恢复。主缸探头无效期间，
...
老四缸低温 N2/N3/恢复阈值为 `<22.5°C`、`<20.5°C`、`>=22.5°C`；小黑缸和毛毛缸为
`<23.5°C`、`<22.5°C`、`>=23.5°C`。三个小缸高温阈值沿用 `>27.5°C`/`>28.5°C`，
